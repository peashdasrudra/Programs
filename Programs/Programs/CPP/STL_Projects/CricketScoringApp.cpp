...
public:
    ...
    int total_runs;
    virtual void update_score_board() {}
    void toss_decision();
    void display()
    {
    label:
        cout << endl
             << "                                                         LIVE SCORING                                                        ";
        cout << endl
             << "Enter Tournament Type" << endl;
        cout << endl
             << "1. Limited Overs series" << endl
             << "2. Test_series" << endl
             << "3. Tri_series" << endl
             << "4. ICC Tournament" << endl
             << "5.back" << endl
             << endl;
    ...
    lebel:
        cout << endl
             << "Want to set the squad ?(y/n)"
             << "  (THIS FUNCTION IS NOT not avilable YET -- press n only)";
        cin >> ch;
        if (ch == 'n' || ch == 'N')
            toss_decision();
        else if (ch == 'Y' || ch == 'y')
        ...
        if (ch == 'H' || ch == 'h')
        {
            if (c % 2 == 0)
            {
                ...
                cout << endl
                     << "Toss won by " << team1;
                cout << endl
                     << "What do you want to do??(ball/bat)";
                cin >> toss_choice;
                cout << endl
                     << team1 << " won the toss and decided to " << toss_choice << " first";
            }
            else
            {
                toss_winner = team2;
                ...
                cout << endl
                     << "Toss won by " << team2;
                cout << endl
                     << "What do you want to do?? (ball/bat)" << endl;
                cin >> toss_choice;
                cout << endl
                     << endl
                     << team2 << " won the toss and decided to " << toss_choice << " first";
            }
        }
        else if (ch == 'T' || ch == 't')
        {
...
public:
    ...
    void update_score_card()
    {
        // system("cls");
        cout << endl
             << "**************************************" << team1 << " vs " << team2 << " Live from " << venue << "**********************************";
        if (ball > 5)
        {
            ball = 0;
            over++;
        ...
        cout << endl
             << team1 << "-" << total_runs << "/" << wickets << "   "
             << "(" << over << "." << ball << "overs)"
             << " Extra :" << ex;
        if (innings == 2)
            cout << endl
                 << endl
                 << team1 << " require " << rem_runs << " runs of " << rem_balls << " balls " << endl;
        getch();
    }
...
public:
    ...
    void update_score_card()
    {
        ...
        cout << endl
             << "*************************************" << team2 << " vs " << team1 << " Live from " << venue << "**************************************";
        if (ball > 5)
        {
            ball = 0;
            over++;
...
void Team1::run_score_board()
{
    ...
    cout << endl
         << " ***********************    Welcome To " << team1 << " vs " << team2 << " Match NO. " << this->series_match_no << " Live from " << venue << "   ****************";
    cout << endl
         << endl
         << this->team1 << "-" << this->total_runs << "/" << this->wickets << "   "
         << "(" << this->over << "." << this->ball << "overs)"
         << " Extra : " << this->ex;
    Sleep(1000);
    cout << endl
         << endl
...
void Team1::next_inning()
{
    ...
    innings = 2;
    cout << endl
         << "WELCOME TO SECOND INNINGS" << endl
         << endl
         << "TARGET : " << target;
    getch();
    T1.run_score_board();
...
void Score_board(y &t)
{
    ...
    while (t.balls < BALLS && t.wickets < 10 && achived == 0)
    {
        ...
        cout << endl
             << endl
             << "R U Ready for the next ball !!" << endl;
        cout << endl
             << "0. Dot Ball" << endl
             << "1. Single" << endl
             << "2. Double" << endl
        ...
        case 7:
            ...
            if (type_w == "runout" || type_w == "RUNOUT" || type_w == "run out" || type_w == "RUN OUT" || type_w == "stumping" || type_w == "STUMPING")
            {
                ...
                {
                    cout << endl
                         << "any extra run in this runout??(0-6)";
                    cin >> extra;
                    t.total_runs += extra;
                    freehit = 0;
                    t.balls++;
            ...
            else
            {
                ...
                {
                    cout << endl
                         << "any extra runs on this free hit ?? (0-6)" << endl;
                    cin >> extra;
                    t.total_runs += extra;
                    t.balls++;
                    t.ball++;
                ...
                {
                    cout << endl
                         << "any extra runs on this no ball ?? (0-6)" << endl;
                    cin >> extra;
                    cout << endl
                         << "It's a freehit" << endl;
                    t.total_runs += extra + 1;
    ...
    if (innings == 1)
    {
        cout << endl
             ...
             << "Innings Over..." << endl;
        cout << endl
             << t.team2 << " REQUIRE " << t.total_runs + 1 << " RUNS of " << BALLS << " BALLS " << endl;
        cout << endl
             << "Wait for next inning...";
        getch();
    }
    else
    {
        if (t.total_runs >= target)
            cout << endl
                 << endl
                 << "*****************" << t.team1 << " WON BY " << 10 - t.wickets << " wickets and " << BALLS - t.balls << " balls reamining ****************" << endl;
        else if (t.total_runs < target - 1)
        {
            cout << endl
                 << endl
                 << "*******************" << t.team2 << " WON BY " << (target - 1) - t.total_runs << "*******************";
        }
        else
            cout << endl
                 << endl
